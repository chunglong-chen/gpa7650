/**
 * @file user_config.h
 * @brief Declarations for user configuration, no-trespass border, and torque settings.
 */
...
#pragma pack(4)
/**
 * @brief Structure for storing user motor configuration.
 */
typedef struct{
...
/**
 * @brief Structure defining the no-trespass border using a quadratic region.
 */
typedef struct{
    bool enable;
    bool greater_than;
...
/**
 * @brief Structure for storing torque configuration settings.
 */
typedef struct{
...
extern USER_CONFIG userConfig[MOTOR_COUNT];
extern NO_TRESPASS_BORDER noTrespassBorder[8];
extern TORQUE_CONFIG torqueConfig[MOTOR_COUNT];
