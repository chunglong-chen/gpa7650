/**
 * @file    sony_ecx336.c
 * @brief   Sony display control task and animation implementation.
 * @ingroup fixation_display_controller
 * @details
 * This module implements the display control logic for the Sony panel,
 * including screen update, full-screen color rendering, animation control,
 * transition handling, and task-based command processing.
 *
 * The module operates as a FreeRTOS task and receives display control
 * commands through a message queue. It also maintains the animation state,
 * display buffer usage, and transition timing.
 */
...
/**
 * @brief   Release transition buffer memory.
 *
 * @details
 * This function frees the memory used by the transition buffer.
 * It is used when transition effect data is no longer needed.
 */
static void sony_free_transition_buffer(void) {
    gp_free((void*)transition_buf);
}
/**
 * @brief   Stop transition effect and release related buffer.
 *
 * @details
 * This function stops the current transition process by releasing
 * the transition buffer resources.
 */
static void sony_stop_transition() {
    sony_free_transition_buffer();
}
/**
 * @brief   Release animation buffer memory.
 *
 * @details
 * This function frees the memory used by the base animation display buffer.
 */
static void sony_free_animation_buffer(void) {
    gp_free((void*)base_disp_buf);
}
/**
 * @brief   Stop animation and release animation resources.
 *
 * @details
 * This function stops the current animation by setting the animation
 * state to stop, restoring the display update flag, and releasing
 * both animation and transition buffers.
 */
...
/**
 * @brief  Render a pattern on the TFT panel based on the provided message.
 *
 * @details
 *   - Validates pattern; returns FALSE if invalid.
 *   - Stops active animation, allocates/clears display buffer.
 *   - Renders by pattern:
 *       FULL: fill with color
 *       RECTANGLE: draw rect at (x,y) size (w,h)
 *       BLINK: init base buffers only (no immediate update)
 *       OPEN/CLOSE/OK: require 120x120; blit bitmap
 *   - Calls update_current_pattern(); for non-BLINK, updates display and frees buffer.
 *
 *
 * @param msg
 *   - x, y           : Top-left origin relative to panel center (signed).
 *   - w_size, h_size : Pattern dimensions (pixels).
 *   - pattern        : One of sony_pattern_e.
 *   - color          : 16-bit color index/value; normalized via sony_check_color().
 *   - speed          : Used by animated patterns (e.g., BLINK).
 *   - brightness     : Pattern brightness if applicable.
 *   - index          : Sequence/index propagated from the caller (if used by upper layers).
 *
 * @return BOOLEAN
 *   - TRUE  on success.
 *   - FALSE on invalid pattern, invalid size for OPEN/CLOSE/OK, or other early validation failure.
 */
static BOOLEAN sony_show(sony_task_message_t msg) {
    ...
    case SONY_PATTERN_OK:
        if(msg.h_size !=120 || msg.w_size!=120){
...
/**
 * @brief   Display a full-screen solid color pattern.
 *
 * @details
 * This function stops any active animation, allocates a temporary
 * display buffer, fills the entire screen with the specified color,
 * updates the display, and then releases the temporary buffer.
 *
 * It also updates the current display pattern information.
 *
 * @param msg Display control message containing color and pattern information.
 */
...
/**
 * @brief   Change transition speed factor.
 *
 * @details
 * This function adjusts the transition step size according to
 * the specified speed multiplier.
 *
 * The input speed must be greater than 0 and less than or equal to 1.
 * If the value is invalid, the function returns an error.
 *
 * @param speed Transition speed scaling factor.
...
/**
 * @brief   Sony display task entry function.
 *
 * @details
 * This task initializes the display device, creates the command queue,
 * sets the default display state, and continuously processes incoming
 ...
 * - Position and size update
 * - Display information query
 * - Transition speed adjustment
 * - Brightness adjustment
 *
 * The task also executes animation and transition logic periodically
